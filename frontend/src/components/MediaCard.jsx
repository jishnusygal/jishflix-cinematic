import { Film, Music2, Play } from 'lucide-react';
import { artwork, minutes } from '../api';
const AUDIO_TYPES = ['MusicAlbum', 'Audio', 'AudioBook'];
export default function MediaCard({item, onSelect, wide = false}) {
  const image = artwork(item, wide);
  const progress = item.UserData?.PlayedPercentage || 0;
  const audio = AUDIO_TYPES.includes(item.Type);
  const subtitle = item.Type === 'Episode' ? `S${item.ParentIndexNumber || 1} · E${item.IndexNumber || 1} · ${item.Name}`
    : item.Type === 'Audio' ? [item.Artists?.join(', ') || item.AlbumArtist, item.Album].filter(Boolean).join(' · ')
    : item.Type === 'MusicAlbum' || item.Type === 'AudioBook' ? [item.AlbumArtist || item.Artists?.join(', '), item.ProductionYear].filter(Boolean).join(' · ')
    : [item.ProductionYear, item.Type === 'Movie' ? 'Movie' : item.Type?.replace(/([a-z])([A-Z])/g, '$1 $2'), item.RunTimeTicks && `${minutes(item.RunTimeTicks)} min`].filter(Boolean).join(' · ');
